  ...
  internal class Builder(private val mContext: Context) {
    ...
    fun icon(iconDrawableId: Int): Builder {
      mBuilder?.setIconBitmap(
        ResourcesCompat.getDrawable(
          mContext.resources, iconDrawableId, mContext.theme
        )?.toBitmap()
      )
      return this
    }
    ...
    fun gridLayout(isGrid: Boolean): Builder {
      ...
      val hints = bundleOf(
        ...
        CONTENT_STYLE_BROWSABLE_HINT to if (isGrid) CONTENT_STYLE_GRID_ITEM_HINT_VALUE
        else CONTENT_STYLE_LIST_ITEM_HINT_VALUE,
        CONTENT_STYLE_PLAYABLE_HINT to if (isGrid) CONTENT_STYLE_GRID_ITEM_HINT_VALUE
        else CONTENT_STYLE_LIST_ITEM_HINT_VALUE
      )
