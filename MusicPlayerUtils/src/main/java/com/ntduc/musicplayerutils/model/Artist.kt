...
data class Artist(
  val id: Long, val albums: List<Album>, val isAlbumArtist: Boolean = false
) {
  constructor(
    artistName: String, albums: List<Album>, isAlbumArtist: Boolean = false
  ...
  var name: String = "-"
    get() {
      ...
      return when {
        MusicUtil.isVariousArtists(name) -> VARIOUS_ARTISTS_DISPLAY_NAME
        MusicUtil.isArtistNameUnknown(name) -> UNKNOWN_ARTIST_DISPLAY_NAME
        else -> name!!
      }
    }
  ...
  val sortedSongs: List<Song>
    get() {
      ...
      return songs.sortedWith(when (PreferenceUtil.artistDetailSongSortOrder) {
