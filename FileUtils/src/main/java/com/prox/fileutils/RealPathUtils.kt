...
import com.prox.fileutils.NanoIdUtils.randomNanoId
import java.io.File
...
private fun Uri.getDownloadsDocumentPath(context: Context, hasSubFolders: Boolean): String? {
    ...
    if (fileName != null) {
        filePath =
            if (subFolderName != null) Environment.getExternalStorageDirectory().toString() +
...
private fun Uri.getSubFolders(): String? {
    val replaceChars = this.toString().replace("%2F", "/")
